...
/**
 * The dock-height rules: the same sparse, shrink-back storage its two neighbours use, on the one metric that is a
...
class DockHeightTest {
    ...
    fun `nothing stored resolves to the blueprint's height`() {
        assertEquals(base, SurfaceMetrics.Default.dockHeight(phone, base))
    }
    ...
    fun `a stored height applies to its own device configuration only`() {
        ...
        val metrics = SurfaceMetrics.Default.withDockHeight(phone, 140)

        assertEquals(140, metrics.dockHeight(phone, base))
        assertEquals(base, metrics.dockHeight(DeviceConfiguration.PHONE_LANDSCAPE, base))
    }
    ...
    fun `clearing a height removes the entry rather than storing one`() {
        ...
        val metrics = SurfaceMetrics.Default
            ...
            .withDockHeight(phone, null)

        assertEquals(SurfaceMetrics.Default, metrics)
        assertTrue(metrics.dockHeightDp.isEmpty())
    }
