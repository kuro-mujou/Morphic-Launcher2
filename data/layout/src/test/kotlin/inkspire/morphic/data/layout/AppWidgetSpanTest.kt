...
/**
 * Spec for [AppWidgetSpan] — the arithmetic that turns a provider's stated pixel size into a footprint on the lattice.
 *
...
class AppWidgetSpanTest {
    ...
    private fun spanFor(minWidthPx: Int, minHeightPx: Int) =
        AppWidgetSpan.forMinSize(minWidthPx, minHeightPx, cell, cell, config)

    @Test
    fun `an exact fit takes exactly that many cells`() {
        assertEquals(AppWidgetSpan(4, 2), spanFor(400, 200))
    }

    @Test
    fun `a widget needing part of a further cell takes the whole cell`() {
        assertEquals(AppWidgetSpan(5, 3), spanFor(401, 201))
    }
    ...
    fun `a widget smaller than one visual cell still occupies one`() {
        ...
        assertEquals(AppWidgetSpan(2, 2), spanFor(10, 10))
    }
    ...
    fun `a widget larger than the grid is offered at the largest span that fits`() {
        ...
        assertEquals(AppWidgetSpan(8, 10), spanFor(5_000, 5_000))
    }

    private fun widgetSpan(targetCols: Int, targetRows: Int, minWidthPx: Int = 400, minHeightPx: Int = 200) =
        AppWidgetSpan.forWidget(targetCols, targetRows, minWidthPx, minHeightPx, cell, cell, config)
    ...
    fun `a declared target size wins over the min-pixel derivation`() {
        ...
        assertEquals(AppWidgetSpan(4, 4), widgetSpan(targetCols = 2, targetRows = 2, minWidthPx = 401, minHeightPx = 201))
    }
    ...
    fun `a target larger than the grid is clamped to it`() {
        assertEquals(AppWidgetSpan(8, 10), widgetSpan(targetCols = 20, targetRows = 20))
    }
    ...
    fun `a target survives an unmeasured grid, since cells do not enter into it`() {
        assertEquals(
            AppWidgetSpan(4, 4),
            AppWidgetSpan.forWidget(2, 2, 400, 200, cellWidthPx = 0f, cellHeightPx = 0f, config = config),
        )
    }

    @Test
    fun `an unmeasured grid answers nothing`() {
        assertNull(AppWidgetSpan.forMinSize(400, 200, cellWidthPx = 0f, cellHeightPx = cell, config = config))
        assertNull(AppWidgetSpan.forMinSize(400, 200, cellWidthPx = cell, cellHeightPx = 0f, config = config))
    }

    @Test
    fun `the label counts visual cells, not logical ones`() {
        // 4x2 logical on a multiplier-2 grid is the 2x1 a user sees.
        assertEquals("2 × 1", AppWidgetSpan(4, 2).visualLabel(config))
    }

    @Test
    fun `the label never claims a widget occupies no cells`() {
        assertEquals("1 × 1", AppWidgetSpan(1, 1).visualLabel(config))
    }
}
