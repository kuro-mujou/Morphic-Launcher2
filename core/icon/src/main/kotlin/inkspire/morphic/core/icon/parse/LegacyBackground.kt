...
/**
 * Decides whether a legacy icon's edge pixels are *one flat opaque colour*, and which.
 *
 ...
 * a solid coloured plate") the *right* colour was sitting in the artwork all along.
 *
 ...
 * gradients, shadows and anti-aliasing — so the foreground keeps the whole bitmap, colour included, and this only
...
object LegacyBackground {
    ...
    /** Below this alpha a pixel is not "solid colour" — it is the artwork fading out, or nothing at all. */
     ...
     * icon's edge is fully opaque, painting its own edge colour behind it changes not one pixel until the user
     ...
     * than guessed at. The user can still choose a solid colour by hand; what this must never do is change an icon
     * nobody asked it to change.
     */
    private const val MinSolidFraction = 0.95f

    /** How far a channel may sit from the mean and still count as "the same colour". */
    ...
    /**
     * The packed ARGB fill for an edge [ring], or `null` when the edge is not one flat opaque colour.
     *
