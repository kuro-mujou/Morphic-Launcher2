 ...
 *
 * These moved here with [Streamlines.trace] itself, which used to sit on the Flow Field generator; that design no
 * longer traces at all, and the two remaining callers are other generators.
 */
class StreamlinesTest {
    ...
    fun `a flat field to the right steps straight across at the step length`() {
        ...
        val line = Streamlines.trace(startX = 0.1f, startY = 0.5f, steps = 5, stepLength = 0.2f) { _, _ -> 0f }
    ...
    fun `a streamline ends the step after it leaves the frame`() {
        ...
        val line = Streamlines.trace(startX = 0.9f, startY = 0.5f, steps = 10, stepLength = 0.2f) { _, _ -> 0f }
    ...
    fun `the same start and field trace the same line`() {
        ...
        assertTrue(
            Streamlines.trace(0.2f, 0.3f, 8, 0.05f, field)
                .contentEquals(Streamlines.trace(0.2f, 0.3f, 8, 0.05f, field)),
        )
    }
}
