...
class DotGridGeneratorTest {
    ...
    fun `density maps to the column count range`() {
        assertEquals(8, DotGridGenerator.gridColumns(0f))
        assertEquals(26, DotGridGenerator.gridColumns(1f))
        assertEquals(26, DotGridGenerator.gridColumns(2f)) // clamped
    }

    @Test
    fun `a weak field draws no dot, so paper stays clean`() {
        assertEquals(0f, DotGridGenerator.radiusAt(0f), 0f)
        assertEquals(0f, DotGridGenerator.radiusAt(0.1f), 0f) // below the floor
    }

    @Test
    fun `radius climbs with the field above the floor and never exceeds the cell`() {
        val mid = DotGridGenerator.radiusAt(0.6f)
        val strong = DotGridGenerator.radiusAt(1f)
