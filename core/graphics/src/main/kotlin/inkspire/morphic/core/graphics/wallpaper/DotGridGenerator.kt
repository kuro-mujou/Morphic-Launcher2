 ...
 *
 * **A lattice, not a scatter — the opposite of [ConfettiGenerator].** Confetti places dots at random (Poisson) points;
 * Dot Grid pins them to a fixed grid and varies only their *radius*, so the eye reads a halftone screen — bigger dots
 * where the underlying field is strong, dwindling to nothing where it is weak. The field is [PerlinNoise2d], so the
 * swell of large dots wanders organically rather than in a plain gradient. Each dot is also colored by that same field
