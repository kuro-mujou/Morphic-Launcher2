...
/**
 * How long a dragged app must dwell on a collection's merge target before it opens mid-drag to take the app in.
 */
private const val OPEN_COLLECTION_DWELL_MS = 1000L
 ...
 * the one it started in is [AppCollectionHostState.dragSourceCollectionId], which outlives all of them.
 *
 ...
 *   [AppCollectionHostState].
 * @property collectionId the collection this phase concerns, or null in [Closed].
 */
sealed interface AppCollectionPhase<out Id : Any> {

    val collectionId: Id?

    /** No collection is on screen. During a drag this is the app "in transit" over the surface. */
    data object Closed : AppCollectionPhase<Nothing> {
        override val collectionId: Nothing? get() = null
    }

    /** [collectionId]'s overlay is open and idle. */
    data class Open<out Id : Any>(override val collectionId: Id) : AppCollectionPhase<Id>

    /**
     * The phases where [app] is being brought into [collectionId] and is therefore **in neither place's own data**: it is
     ...
     */
    sealed interface Incoming<out Id : Any> : AppCollectionPhase<Id> {
        val app: ComponentKey
        override val collectionId: Id
    }

    /**
     * [app] is mid-drag over [collectionId]: not a member yet, rendered as an extra cell so the drop can report an
     * order that includes it.
     */
    data class Injecting<out Id : Any>(override val collectionId: Id, override val app: ComponentKey) : Incoming<Id>

    /**
     * [app] has been committed to [collectionId], but the store hasn't caught up: the collection's contents still come
     ...
     * [AppCollectionHostState.onMembersChanged] sees it land.
     */
    data class Injected<out Id : Any>(override val collectionId: Id, override val app: ComponentKey) : Incoming<Id>
}
 ...
 *
 * It lives here, beside [AppCollectionOverlay], because none of it is surface-specific: a collection is opened, reordered,
 ...
 *   answer ([rememberAppCollectionHostState]): a coordinate surface compares placements, an ordered one compares slots, and
 ...
 * latched for the rest of the drag, which is why [AppCollectionPhase] has no "leaving" state: leaving *is*
 * [AppCollectionPhase.Closed].
 *
 ...
 *
 * A second exclusion used to be listed here: the open collection's published `AppCollectionDragDelegate`, which a surface
...
class AppCollectionHostState<Id : Any> {
     ...
     *
     * The type argument is spelled out rather than inferred: [AppCollectionPhase.Closed] is an
     * `AppCollectionPhase<Nothing>` (there is no collection for it to name), so an inferred delegate would narrow
     * this state to that one phase.
     */
    var phase: AppCollectionPhase<Id> by mutableStateOf<AppCollectionPhase<Id>>(AppCollectionPhase.Closed)
        private set

    /** The on-screen collection's id, or null when none is — true of every phase but [AppCollectionPhase.Closed]. */
    val openCollectionId: Id? get() = phase.collectionId

    /**
     * The app being brought into the open collection that its own data can't render yet — mid-drag *or* just
     * committed (see [AppCollectionPhase.Incoming]). Null otherwise.
     */
    val incomingComponent: ComponentKey? get() = (phase as? AppCollectionPhase.Incoming<*>)?.app
     ...
     * first.
     *
     * Deliberately *not* a field on [AppCollectionPhase]: it is a property of the **drag**, not of where the
     * interaction is, and the two are independent (which is why it doesn't reintroduce the flags-that-must-agree
     * problem the phase was built to remove). The phase cannot answer it either — it names whichever is on screen
     * *now*, which after one hand-off is no longer the one holding the pointer stream.
     *
     ...
     * needs to be: what the drag still owes is [dragSourceCollectionId], which was fixed when it started.
     */
    fun leaveCollection() {
        phase = AppCollectionPhase.Closed
    }
     ...
     * [AppCollectionPhase.Incoming], because the write hasn't come back yet: its contents don't include the app, so
     * releasing it here is what made it blink out. [onMembersChanged] finishes the hand-off.
     */
    fun injectCommitted() {
        val current = phase
        if (current is AppCollectionPhase.Injecting) {
            phase = AppCollectionPhase.Injected(current.collectionId, current.app)
        }
    }
    ...
    fun onMembersChanged(members: List<ComponentKey>) {
        val current = phase
        if (current is AppCollectionPhase.Injected && current.app in members) {
            phase = AppCollectionPhase.Open(current.collectionId)
        }
    }

    /**
     * A drag began. If a collection is on screen the drag necessarily started **in** it — its scrim covers the
     * surface, so no cell underneath can receive the press — and that one is [dragSourceCollectionId] for the rest of the
     * gesture.
     */
    fun onDragStart() {
        dragSourceCollectionId = openCollectionId
    }
     ...
     */
    fun onDragEnd() {
        dragSourceCollectionId = null // scoped to one drag; no pointer stream left to preserve, so the holder can go
        phase = when (val current = phase) {
            is AppCollectionPhase.Injecting -> AppCollectionPhase.Closed
            is AppCollectionPhase.Injected, is AppCollectionPhase.Open, AppCollectionPhase.Closed -> current
        }
    }
}
 ...
 * - **open-to-inject**: holding a dragged app on a collection's merge target for [OPEN_COLLECTION_DWELL_MS] opens it
 ...
 * - **drag boundaries**: the collection the drag started in is captured on lift ([AppCollectionHostState.onDragStart]) and
 *   every hand-off is cleared on release ([AppCollectionHostState.onDragEnd]).
 *
 ...
 * @param collectionIdAt resolves a merge plan **in a given zone** to the collection it targets, or null when the target
 ...
 */
@Composable
fun <Id : Any> rememberAppCollectionHostState(
    coordinator: DragCoordinator,
    collectionIdAt: (ZoneId, PlacementPlan) -> Id?,
): AppCollectionHostState<Id> {
    val host = remember { AppCollectionHostState<Id>() }
    ...
    val mergeTargetId: Id? = run {
        ...
        if (host.openCollectionId != null) return@run null
        collectionIdAt(zone, plan)
    }
    LaunchedEffect(mergeTargetId) {
        val collectionId = mergeTargetId ?: return@LaunchedEffect
        delay(OPEN_COLLECTION_DWELL_MS.milliseconds)
        val app = (coordinator.session?.item as? GridItem.App)?.component ?: return@LaunchedEffect
        host.beginInject(collectionId, app)
    }
