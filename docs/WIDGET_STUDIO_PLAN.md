...
twelve-design template library, shown live in the picker. **WS7a (blocks and selection) and WS7b (drag, pinch, remove)
are done;** WS7c (the block library) is next. The third studio, after the icon studio (done) and the
...
| **WS7c** | **The block library** | Blocks offered to insert into a placed widget, each landing where there is room and then moved with WS7b's drag; bring forward / send back. The templates are rebuilt from the library's blocks, so a block and the template using it cannot drift. |
