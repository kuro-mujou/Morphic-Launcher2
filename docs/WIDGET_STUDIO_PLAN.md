...
twelve-design template library, shown live in the picker. **WS7 (blocks, direct manipulation, the block library) and WS8a
(the layer stack, behind Advanced) are done;** WS8b (Stack, Series, image import) is next. The third studio, after the icon studio (done) and the
...
| **WS8b** | **Stack + Series** | The `Stack` container (flow along an axis) and the `Series` source; Image import. |
