...
| 17 | Polygon Cascade | `POLYGON_CASCADE` | ✅ | **W11w** — theirs marches its copies across the frame between two centres where ours stacked every one on a fixed centre and drew a rosette; nine knobs, not six, and both of its "delta" knobs are **totals** rather than per-copy rates. `rotation` added |
