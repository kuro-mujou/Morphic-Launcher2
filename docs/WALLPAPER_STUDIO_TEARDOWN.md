...
| 15 | Topography | Style · **Color mode** · Coverage · Levels · Zoom · Variation · **Detail · Smoothing · Thickness · Highlighted level · Thickness boost** — **eleven** (W11u), five past the fold and the longest tab row in the studio. *Style* is segmented **Embossed / Contour lines** (default lines). *Color mode* here is **not** the global one — it is a color *distribution*: `Altitude · Islands · First color · **Hills** · Random`, the third design whose "Color mode"/"Color distribution" tab means something of its own. *Levels* **1..10** (default **4**) is iso-lines, not bands, and each is a stroked path. *Coverage* **10..100** (79) is how much of the elevation range the levels are spread across — at `10` they pack into a slice and only the ground crossing it is drawn. *Zoom* **10..100** (77) is feature size; at `100` barely one hill spans the frame. *Variation* **0..100** (80) is the relief's **amplitude** — hill outlines stay put, ring counts change. *Detail* **0..100** (49) is the fBm octaves. *Smoothing* **0..100** (25) rounds a traced polyline, and end to end it moves lines a pixel or two. *Thickness* **2..100** (12) measures ~1.5px, ~3px and ~17px on a 1080-wide frame. *Highlighted level* is **None plus one entry per level**, so its range moves with *Levels* (default `2`), and *Thickness boost* **120..400%** (240) is how much wider that one is drawn |
| 16 | Ribbed Glass | Real glass · Count · Complexity · Refraction · Vibrancy — **five, and the row really does stop there** (W11v; the first design in this pass whose recorded count was already right). *Real glass* is segmented Off/**On** and is **not a sub-look** — turn it off and the tab row becomes six *different* knobs (Real glass · Count · **Light angle · Contrast · Thickness** `28` **· Softness** `0`) over a striped satin drawn from a one-dimensional ramp, with only *Count* shared. *Count* **1..30** (default **7**), and at `1` there are no ribs at all — the bare background, which is what proves the background is a real generated field. *Complexity* **1..5** (2) is how many color blobs that field holds; at `1` it collapses to a plain diagonal ramp. *Refraction* **0..200** (50) is the lens, and `0` is undistorted field with only a hairline seam per rib; driven up, the colors inside a rib get **larger** — it magnifies. *Vibrancy* **0..100** (50) is a vividness grade on the field (saturation `0.61 → 0.84`, value `0.54 → 0.75` across its travel), not a geometry knob. The rib bevel is tiny: measured at *Refraction* `0`, `+7%` just inside a rib's leading edge decaying over a fifth of it, and a hairline `−8%` seam at its trailing edge |
| 17 | Polygon Cascade | Shape · Mode · Thickness · Iterations · Rotate delta · Size · **Scale delta · First shape center · Last shape center** — **nine**, not six (W11w), the last three past the fold and the last two a four-arrow **nudge pad** each. **It is a tween between two fully-specified shapes**: the two pads are the first and last copy's centres (randomized on every pick, and they point any direction — a diagonal at one pick, flat across the frame at the next), *Size* is the first copy's size and *Scale delta* and *Rotate delta* are the **last** copy's, not per-copy rates. That is what *Iterations* **1..100** (default **10**) gives away: driven `10 → 17` the ink's bounding box is identical to the pixel, so the knob only subdivides. *Size* **1..100** (15–16) is `0.0435 × S` of the frame width, measured the same on two picks. *Scale delta* **1..100** (4) is the whole cascade's size change — `≈ 0.050 × (18.9 − D)` of the first copy — so it passes through a **neutral at ≈18** where every copy is the same size and grows beyond it; which end is the larger one is the pick's, not the knob's. *Rotate delta* **−180..180** (57) is the total turn, confirmed exactly twice (`9 → 9.01°`, `12 → 12.02°` across seventeen copies). *Shape* is a named vocabulary of six — Circle · **Star** · Triangle · Hexagon · Square · Rectangle — with the star's inner radius `0.451` of its outer (not the pentagram's `0.382`) and the rectangle `2:1` with corners at `0.3` of its short side. *Mode* Stroke / **Fill** swaps a knob: *Thickness* **1..100** (5, measuring `0.0025 × T` of the frame width) under Stroke, *Shadow* **0..100** (**0**) under Fill — and that shadow is a blurred silhouette behind each copy with **no offset**, radius fixed at ~6.5% of the frame width with the knob on its opacity. Ground is stop 0, the copies the ramp above it, spent over the cascade's length |
| 18 | Soft Overlaps | Count · **Blend mode** · Mode · Position jitter · Radius · Size variation · **Complexity · Irregularity · Distance from centre · Blur** — **ten**, not six (W11x). **Its shapes have hard edges**: four enormous flat forms whose silhouettes stay crisp while their *colors* mix where they cross — the "soft" is the shapes being big and organic, not their edges. *Mode* is **Fill** / Glow and the glow is the soft-falloff look. *Count* **1..10** (default **4**), and `1` — one form alone on the ground — is a real setting. **A form is a closed curve through *Complexity* points around an ellipse, each pushed off its radius by *Irregularity***: `3..16` (8) and `0..100` (25), and at *Irregularity* `0` every form is an **exact ellipse** while *Complexity* does nothing at all, which is what proves the construction. *Blend mode* is **nine** — Normal · **Screen** · Lighten · Plus · Overlay · Multiply · Darken · Color Burn · Color Dodge — and opening on Screen is most of why their overlaps glow. *Radius* **60..400** (300); driving it **re-rolls the shape**, so two settings are not the same form and their bounding boxes cannot be compared. *Size variation* **0..100** (40) shrinks rather than spreading both ways; `0` draws every form identical. *Position jitter* **0..100** (50) is the scatter off a regular arrangement. *Distance from centre* **0..50** (0) pushes every form outward to hollow the middle, and *Blur* **0..100** (0) is a grade over the whole composed picture |
| 19 | Rounded Tiles | Count · Margin · Spacing · **Blend mode** · Rotation · **Direction · Inner shadow** — **seven**, not six (W11y). **It is a fan of long rounded bars, and *Direction* is what makes it one**: at its default `0` every bar is parallel and the picture is a stack of stripes, which is why the name looks wrong until the knob moves; climbing it gives each bar its own angle so they radiate and the rounded caps swing into frame. *Count* **1..10** (7) — the bars always fill the frame, so it sets their thickness too, and `1` draws a single bar across the whole frame. *Margin* **0..100** (20) **shortens each bar from both ends, and at `100` every one has become a circle** — which is the proof the shape is a *capsule*, since only a bar whose cap radius is half its thickness degenerates that way. *Spacing* **−50..50** (44) is **signed**: its negative end overlaps the bars into one another and, under the default blend, blows the frame out to a bright wash. *Blend mode* here is **two** options — Normal · **PLUS** — where Soft Overlaps' is nine, so the name means something different again; it shows only where bars overlap. *Rotation* **0..100 → 0..180°** (0) is the whole fan's aim, measured by tracing one gap across the frame (`11 → 19.79°`, `23 → 41.38°`, both exactly `1.8×`). *Inner shadow* Off/**On** is a **soft** inset just within each bar's edge, profiled at `×0.72` three hundredths of the thickness in and back to `×1.00` by a fifth — an ease, not a band. Every bar also carries a gradient along its own length |
| 20 | Ribbon Flow | Count · Distortion · Rotation · Thickness · Detail · Gradient offset — six, and the construction is in the first two (W11z). **It is a ruled rank of parallel lines combed off their lanes by a noise field**: at *Distortion* `0` they are exactly straight and exactly evenly spaced, which is the rigid end that settles it. *Count* **10..50** (30) is the rank's size, not what you can see — the spacing is `extent / (count − 1)` exactly (solved against five counts to <0.1%) over an extent ~`1.32 ×` the frame height, so ~a quarter of the lanes start off-frame; measured again on a second frame shape, the extent scales with the height and does **not** change with the rotation. *Thickness* **0..100** (12) is a **percentage of the lane**, not a width — at `100` the strokes exactly touch and no ground shows, and the stroke stays that width where lines bunch (so these are stroked paths, not a field-metric band). *Distortion* **0..100** (50) is the sideways amplitude, and its maximum sits right on the bound where neighbouring lines would cross (measured: local spacing down to `0.09 ×` nominal). *Detail* **1..20** (8) is the noise frequency — `1` is a single swell. *Rotation* **0..90** (45) is linear in **screen** degrees, but its ceiling moves with the frame (`50.5°` on 1080×2400, `42.3°` on 1080×1600) and the tilt's sign differed between instances, so no isotropic, unit-square or fixed-canvas model fits it. *Gradient offset* **−10..10** (0) slides the ink ramp along its axis and **clamps** — both ends flatten the frame to one tone. The ink is **one gradient across the whole frame** whose axis is the base line direction, shared by every line (iso-colour lines measured perpendicular to the strokes at two rotations and at full distortion); ground is the darkest stop, flat |
| 21 | Flow Lines | Iterations · Complexity · Thickness · Delta rotation · Start · End · **Blur section** — **seven**, not six (W11aa), the last one past the fold. **There is no flow field in it.** It is a tween between two fully-specified states, exactly as the Polygon Cascade is, on an **open curve** instead of a closed shape: *Iterations* **1..100** (40) at `1` draws a single curve and at `100` draws an envelope identical in extent to `40`'s, so the count only subdivides. *Complexity* **0..20** (10) is the base curve's waviness and its `0` is a **dead straight line**; the ruler is about five clicks to the wave — `10` carries roughly two over the curve's length and `20` roughly four, which is a fifth of what the number says. *Thickness* **1..10** (1.8), measured `2.56px` per unit on a 1080-wide frame (~4.6px at the default, ~26px at the top), its ruler labelled with an odd second number (`1/100`, `1.6/1`, … `10/10`) that the pixels do not follow. *Delta rotation* **10..500°** (150°) is the **total** turn, first copy to last, and floors at `10` rather than `0`. *Start* and *End* are four-arrow **nudge pads** — the first and last copy's centre, the cascade's pair again. *Blur section* None/Start/End/**Both** rendered **byte-identical** at None and at Both, twice, at two very different configurations. The palette runs **along each curve's own arc length**, the same ramp on every copy — proved by three copies carrying three different stops across one band of the frame, and by the colour bands at `100` following arc position rather than screen position |
| 22 | Shape Trail | Path style · Length · Roundness · Variation · Thickness · Depth — never driven, and **not being built** (see the checklist below); listed so the inventory of what they ship stays complete |
...
- **Spacing / gaps** — *Spacing, Margin, Offset, Coverage, Size, Radius*. How much air between elements, and how much
  ...
  full-bleed field is the bigger of the two questions by a distance. **The squeeze got its second field in W11w —
  `taper`** — for the shape it takes on a design laid out along a *run*, where a size is not one number but two: the
  cascade's *Size* and *Scale delta*, Neon Ribbons' *Start area* and *End area*, Gradient Columns' *Start spread* and
  *End spread*. It was forced rather than chosen: the cascade's taper had been parked on `depth`, and `depth` is
  needed for the shadow. **W11x widened it from a *run* to a *spread*** — Soft Overlaps has no run at all and spends
  it as that design's *Size variation*, a random departure rather than an ordered one. Both are "the elements are not
  all one size" and both are `0` when they are; that the first consumer ordered its departure along a line was a fact
  about the cascade rather than about the field.
- **Orientation** — *Rotation, Direction, Delta rotation, Rotate delta*. Which way it points / turns. **Built as
  `rotation` (W11w)** — the last family to get a field, and it had been held back on purpose: four of ours spend
  `variant` on a discrete direction as a stand-in (five sampled angles on Diagonal Bands, three on Louvers), so a
  field shaped by one consumer risked being the wrong shape for the four that would move onto it later. What settled
  it is that the Polygon Cascade's turn is **continuous and signed**, which a discrete chooser cannot express at all —
  so the field has to be a fraction, and that is one thing the four stand-ins could never have told us. **The four
  stay on `variant`**, for the reason the three `colorLayout` ones did: a stored recipe's `variant` would keep its old
  number and be read as a look it never meant. Moving them is still a slice of its own.
...
- **Rendering / depth** — *Blend mode, Contrast, Shadow, Refraction, Vibrancy, Tridimensionality, Real glass*, and
  ...
  **Confetti's depth of field (W11f)** — the first that is depth rather than lighting. **Built as `depth` (W11h)**, on
  ...
  consumer — it rides `variant` today, which is why that design has no sub-look left for anything else. **Its discrete
  half is `finish` (W11w)** — the members of this family that are a *choice* rather than an amount (*Blend mode* on
  Soft Overlaps and Rounded Tiles, the cascade's *Mode*, Ribbed Glass's *Real glass*) cannot ride a fraction, and
  putting them on `variant` costs a design its sub-look, which is the trap Confetti is already in. **Its second
  consumer landed in W11x**, and it is the one the field was predicted on: Soft Overlaps' *Blend mode*, at four of its
  nine options, with that design's *Mode* on `variant` beside it. **Its third landed in W11y** — Rounded Tiles' own
  *Blend mode*, which is two options where Soft Overlaps' is nine, so the tab name is no guide to the control.
...
| 15 | Topography | `CONTOUR` | ✅ | **W11u** — theirs traces and **strokes** its contours, where ours inked every pixel whose band differed from a neighbour's; a mask is one pixel, one weight, one ink, so *Thickness*, the **highlighted level** and per-contour color were all unreachable by construction. Eleven knobs, not six. Ours also had no *Zoom* and no *Coverage*, and scaled its noise by the frame's **height** on the `y` axis, which stretched every hill by the aspect. The relief's shadow is a **thrown, blurred silhouette**, not a ramp across each band — the first attempt read as an inner shadow on every shape |
| 16 | Ribbed Glass | `RIBBED_GLASS` | ✅ | **W11v** — what the glass is *over* is half the design and ours had a gradient there with no way to hold anything else; their *Complexity* runs a field of color blobs, and only its `1` is the ramp ours always drew. The lens ran the **wrong way** too (ours squeezed a quarter of the frame into each rib, theirs magnifies) and its `0` was floored on purpose, throwing away their calmest setting. Their *Vibrancy* went to the **Filters** stage. Their *Real glass* Off is a second renderer (six different knobs over a striped satin) and is **not being built, by the author's call** |
| 17 | Polygon Cascade | `POLYGON_CASCADE` | ✅ | **W11w** — theirs marches its copies across the frame between two centres where ours stacked every one on a fixed centre and drew a rosette; nine knobs, not six, and both of its "delta" knobs are **totals** rather than per-copy rates. `rotation`, `finish` and `taper` added, and `Generator.styleFor` widened from a variant index to the whole `DesignParams` |
| 18 | Soft Overlaps | `SOFT_OVERLAPS` | ✅ | **W11x** — ours was their *Glow*, standing in for the whole design, so their default was unreachable; ten knobs, not six, and their form is a deformed ellipse where ours was a faded circle. `taper` widened from a run to a spread |
| 19 | Rounded Tiles | `ROUNDED_TILES` | ✅ | **W11y** — built **beside** ours rather than over it: theirs is a fan of separate capsules, ours a maze of arcs that *join*. Seven knobs, not six. Catalog **30** |
| 20 | Ribbon Flow | `RIBBON_FLOW` | ✅ | **W11z** — a ruled rank of parallel lines displaced sideways by a noise field, not a scatter of streamlines; six knobs, not two, and the ink is one frame-wide gradient every line reads |
| 21 | Flow Lines | `FLOW_LINES` | ✅ | **W11aa** — a tween of one open curve between two placements, the Polygon Cascade's construction; ours was a Perlin flow field with two thousand streamlines in it |

**Every design in scope is driven.** The twenty-second, **Shape Trail**, is **not being built, by the author's call** — a 3-D tube or knot, the only one of theirs with nothing of ours standing in for it, and the only one whose look is real dimensional rendering rather than a flat composition. It is off the list rather than at the bottom of it, for *Real glass*'s reason one design over: a deferred item nobody intends to build reads as work outstanding forever.

**Ours-only designs have no reference to drive and are not in the count:**
`LINEAR_GRADIENT`, `PLASMA`, `MONDRIAN` (split out of Bauhaus by W11a, and *still* ours-only — W11k built theirs beside
...
**What they taught, in one line each, because it is what predicts the next find:** check whether ours *is the same
design* before judging its quality (four of the first six were not); drive *every* knob to *both* ends before
concluding; measure pixels rather than trusting the eye; and the rigid end of a knob is the strongest evidence about
the machinery underneath. **W11u adds a fifth: ask what their knobs imply their *renderer* is.** Topography's
*Thickness*, *Smoothing* and *Thickness boost* are three knobs no pixel mask can answer, so the mask was refuted
before a single pixel was compared — a knob list is evidence about machinery as much as a rigid end is.

| Theirs | Ours | Verdict & fix |
|---|---|---|
| Topography | **Contour** | ✅ **W11u.** The gap was the **renderer**, not the look: a contour of theirs is a traced polyline **stroked** at a width, with one level drawn 2.4× wider than the rest, where ours was a one-pixel band-boundary mask that could carry no width, no highlight and no per-line color. Rebuilt on marching squares over a shared terrain, with their *Coverage* (the elevation window the levels span — where their negative space comes from), their *Zoom*, their measured *Thickness*, and a `colorLayout` of **Hills · Altitude · Random**; the filled look keeps the per-pixel path and gained their measured shadow — **a drop shadow, offset and blurred, not a darkening that rides each band's own edge**: the silhouette of everything higher, thrown ~28px **up and to the right** (their light is from the lower left), soft over another ~30px, multiplying to ×0.80 where solid. A first pass darkened a pixel by how near it sat to the level above, which puts the rim *inside* every ring and gradients each band across its whole width — an inner shadow on every shape rather than one sheet lying over another, which is what the author caught. Two more things the relief needed and the map does not: it runs the palette ramp the **opposite** way (a relief's peaks are the sheets catching the light, so high is palest, where a survey map's highest contour is its strongest ink), and it reads that ramp at **one rung per band** — seven flat sheets over the bichromatic default's three tones merge into one, so `RampTones.countFor` gained a design-supplied floor beside the palette's. Their *Variation* folds into Coverage (the same axis inverted) and their *Smoothing* is not ported. |
...
| Polygon Cascade | **Cascade** | ✅ **W11w.** The identity finding again, and it is in the two knobs the one-line note never had: theirs interpolates a shape's **centre** from a first position to a last, so the copies march across the frame, where ours stacked all of them on one fixed centre and drew a concentric rosette. Same ingredients, different picture. Underneath that, the design is a **tween between two fully-specified shapes** — *Scale delta* and *Rotate delta* land the **last** copy on their number whatever the count, which is why *Iterations* leaves the bounding box identical to the pixel — where ours multiplied a fixed shrink and a fixed turn by the index, so its count re-composed the whole picture. Its shape knob was a **side count** (`3..8`) against their vocabulary of six named shapes, so it could draw neither a circle, a star nor a rectangle and offered three polygons theirs has not; rebuilt on a ring of angle-radius pairs, with the star's inner radius and the rectangle's aspect measured off the reference rather than assumed. Five of ours' fixed constants became knobs, one of them the **orientation** family's new `rotation` field. Their *Mode* is built too, on a second new field — `finish`, the **discrete half of the rendering family** — because the cascade wants a shape *and* a mode at once, and one chooser would have to offer their twelve combinations, which a segmented control dividing its width evenly cannot show at a readable size. Stroked, the whole cascade shows through itself; filled, it is overlapping paper and only each copy's leading sliver survives. Their *Shadow* is built too, and it is the knob that made the seam move: it belongs to the **filled finish alone**, which `styleFor(variant: Int)` could not say — so that signature widened to take the whole `DesignParams`, and a knob set can now hang off any choice rather than off the variant only. The shadow itself is a **blurred silhouette behind each copy with no offset**, which is what scanning a copy's left and right edges settled (the same falloff mirrored, which no throw can do); the radius is fixed at ~6.5% of the frame width and the knob is its opacity, as theirs is. It lands on `depth` — the field four other designs already spend on a knob they each label *Shadow* — which is what displaced the taper onto the new `taper` field, its family having been *size* all along. One of theirs is still not built: their *Thickness* has no field left, `scale` being spent on *Size*, and the coverage wins as it does everywhere in that family. Their two nudge pads are seeded, since theirs re-randomizes the pair on every pick. |
| Rounded Tiles | **Bars** | ✅ **W11y.** The verdict's own hedge was right and understated it: theirs is a **fan of separate capsules**, each turned a little further than the last so the set radiates, where ours tiles the frame with quarter-arcs meeting every cell edge at its midpoint so they *join* into a maze. Not a weaker version of one design — two designs — so this is **built beside** `TRUCHET` rather than over it, as the Mondrian was beside Bauhaus and the Halftone beside Dot Grid, and no stored recipe changes meaning. Two measurements carried the build: *Margin* at `100` turns every bar into a **circle**, which is only true of a capsule whose cap radius is half its thickness, and *Rotation* traced out at exactly `1.8×` its knob, so `0..100` is `0..180°`. Their *Spacing* is **signed** and its negative end overlaps the bars — the only place their blend knob shows anything, which is why ours' spacing range runs past `1` too. The squeeze this time was in the **orientation** family: *Rotation* and *Direction* are both angles and there is one field. The aim took `rotation`, being the field's own meaning, and the **fan went to `irregularity`** — a liberty, since a fan is an *ordered* departure rather than noise, but that field's contract is `0` = rigid and climbing = departure, which is exactly what it does, and this design has no other organic axis competing for it. **The fan itself was built wrong first and looked cheap**: turning the *frame* per bar swings each centre round the middle as well as the bar, so the rank splays into an arc that never crosses itself and the blend knob is left with nothing to combine. Isolated on three thin bars, theirs plainly turns each bar **about its own centre** with every centre staying on the lane line — which is what makes the beams cross and light up. Two more things came out of that comparison: the lanes must be spread across the frame's extent **along the aim**, not its diagonal, or a vertical rank shows two and a half bars each three times too fat; and their bars **overrun the frame** at the default, where a bar ending inside it reads as an object lying on the picture rather than a beam crossing it. Their *Inner shadow* was first built from a glance and looked cheap — a hard stroke at a flat alpha reads as an outline drawn round every bar, and no width fixes that. Profiling theirs gave a smooth ease, `×0.72` three hundredths of the thickness in and clear by a fifth, which ours now matches within a few percent; the non-obvious number is that a `BlurMaskFilter` reaches about **four times its radius**, which washed the whole bar in two earlier attempts. |
| — | **Plasma** | Ours only (SL lacks). Keep. It was grouped here with Rings and Rays as "our loudest, keep but restrain"; the other two turned out not to be restrainable and are **dropped** — see finding 11 below. |
...
| Ribbed Glass | **RibbedGlass** | ✅ **W11v.** Two things were wrong and neither was the ribs. **The background is a field, not a gradient** — their *Complexity* `1..5` is a count of color blobs and only at `1` does it become the diagonal ramp ours could never leave; rebuilt on `ColorLattice` (its third consumer), frame-shaped so the blobs are round, with node colors **blended** toward free positions rather than jittered around the diagonal, because a jitter washes the contrast out as the lattice gets finer. **And the lens bends the other way**: ours fanned a rib across a window up to a quarter of the frame wide (a 4× squeeze), where driving their *Refraction* up makes the colors inside a rib *larger*. Rebuilt on gart's own glass arithmetic (`glass/glassBall.kt`), Snell's law across the rib, with `0` meaning no lens at all — the rigid end our old mapping deliberately refused to have. The bevel was measured and turned out to be a bright leading edge and a hairline seam at about `±8%`, against ours darkening every rib to *half* brightness. Their *Vibrancy* is a whole-image grade and went to the Filters stage; their *Real glass* Off is a second renderer with its own six knobs, and the author's call is **not to build it** — Ribbed Glass is one design here, the refracted one. |
| Ribbon Flow | **RibbonFlow** | ✅ **W11z.** The identity finding an eighth time, and this one had been predictable from the file itself: ours was written in W8c from a one-line note and traced [Streamlines] from **random** starts for a fixed eighty steps, stroking each at a twentieth of the short side with its **own** ramp offset along its **own** arc length. So it drew a scatter of short broad ribbons — a fatter Flow Lines — where theirs rules a rank of full-frame parallel lines and combs them off their lanes, and no setting of ours could draw a straight line at all. Rebuilt on that: lane `i` is drawn at `vᵢ + amplitude · noise(u, vᵢ)` in the rank's own frame, which makes crossing **impossible by construction** — the noise is read at the lane's own `v`, so the ordering holds while `amplitude · ∂noise/∂v > −1`, a bound on the amplitude alone that a streamline tracer or a marching-squares walk of the level sets (both of which draw the same family of curves) would have to hope for instead. A first pass **stretched the field four times across the rank**, on a reading of the reference that turned out to be the noise's own periodicity rather than anisotropy; measuring the structure *down* a column found a period within a fifth of the one along a line, and the isotropic field the correction gives lands the bound's maximum on about the deviation the reference actually draws. Ours **ranks on the frame's diagonal** rather than on the reference's height-derived extent, because an extent that shrinks as the rank turns makes *Rotation* a second density knob; and the amplitude is capped at a share of that diagonal, which is the same share the rank is ruled wider by, so the two cannot disagree about how far a line may wander. Two of theirs are not built as they have them: *Detail* is carried **inverted** on `roundness` (whose `0` must be the sharp end) and named *Smoothness*, and *Gradient offset* is left out — no field fits it, and both of its ends flatten the frame to one tone. Their *Rotation* is taken at its word as a clean `0..90°` in frame space rather than reproduced; see the row above for why it cannot be. |
| Flow Lines | **FlowLines** | ✅ **W11aa.** The identity finding a ninth time and the largest of them: **there is no flow field in the design at all.** Ours combed a [PerlinNoise2d] field with up to two thousand streamlines seeded on a jittered lattice and stroked at one hairline width — a brushed grain over the whole frame. Theirs is a **tween between two fully-specified states**, which is the Polygon Cascade's construction (W11w) on an **open curve** instead of a closed shape, and its *Iterations* `1` settles it by drawing the base curve alone. Rebuilt on that: the curve is a straight span bent by a seeded harmonic sum, laid **square to** the run so the copies fan rather than stack, each copy centred `t` of the way along the run and turned `t` of the way through the total. The knob list gave one number away — taking their *Complexity* `10` as ten waves drew a tight zigzag where the reference draws a long, lazy S, and measuring their own base curve put the ruler at about five clicks to the wave. The palette runs along each curve's arc length rather than across the frame, which is why the ink is a colour per segment and not a shader: a `LinearGradient` projects onto a straight axis and this ramp follows a bending path. Their *Blur section* is not built — driven None ↔ Both twice it rendered **byte-identical**, and a knob that is only meaningful through the Filters stage's own pass is not one to copy on a guess. Two things came out shared, this being the cascade's sibling and so the second call site for both: `TweenRun` (the seeded heading-and-run behind their two nudge pads) and `SeededHarmonics` (the low harmonics at seeded phases, drawn whatever the amplitude, so a knob cannot shift the stream under the picture). |
| Soft Overlaps | **Overlaps** | ✅ **W11x.** The identity finding a seventh time, and this one was hiding in the name: the *soft* in *Soft Overlaps* is the **shapes** — big, round, organic — and not their edges. Theirs lays four enormous flat forms on a dark ground with every silhouette crisp and the *colors* mixing where they cross; ours faded each of seventeen discs to nothing at its rim, so it had no silhouette anywhere and read as a bokeh blur. Driving *Mode* settled it beyond argument — its second look, **Glow**, is exactly what ours drew at every setting, so ours was not a poor version of the design but their *alternate* look standing in for the whole of it. Rebuilt on their construction: a closed curve through eight points around an ellipse, each pushed off its radius, smoothed by a **Catmull-Rom** spline — which is through the points rather than past them, and the distinction is the measurement, since quadratics through the midpoints draw a rounded polygon inscribed in the ring where the reference draws an exact ellipse. Their *Blend mode* is the second consumer `finish` was added for, at four of its nine (a segmented control divides its width evenly); their *Size variation* is the second consumer of `taper`, and **widened it from a run to a spread**, which is the shape that field should have had. Three of theirs are deliberately not built: *Complexity* (both shape fields are spent, and the deformation is the identity), *Blur* (a whole-image grade, and the **Filters** stage already has one — the argument that sent *Vibrancy* there), and *Distance from centre* (a small composition knob with no field spare). And **gart is not where this one lives**: its `blob/Blobs.kt` is a metaball field, our Metaballs' family, and its `deformPath` inserts midpoints and doubles the point count, so at zero offset it leaves an N-gon where theirs leaves an ellipse — the `arts/monet` citation in our KDoc was a brush-stroke impasto sim and is gone rather than re-pointed. |

---

### The ours-only quality pass — the nine with no reference to drive

Every W11 slice skipped these nine, because the method was *drive the reference against ours* and there is no
reference. The method here was different: render each at its default and across every knob it declares, judge against
the five aesthetic principles above, and **measure** whatever can be measured rather than judged.

**What it found, in order of how objective the finding is:**

| # | Finding | Fix |
|---|---|---|
| 1 | **Five of the nine measured their geometry in the unit square.** `nx` and `ny` are each a share of *their own* side, so any `hypot` or `atan2` of the two runs in a space the frame stretches — by 2.22× on a 1080×2400 phone. It is the bug `ContourGenerator`'s KDoc names and W11u fixed there; nobody had reached these. Rings drew **ellipses**, at every setting, forever. Rays took its bearing from that metric, so an even fan arrived as a couple of broad blocks and a handful of slivers. Plasma drew every swell twice as tall as wide. Halftone read a unit-square field through a frame-shaped dot lattice. Voronoi is the pair that must agree — seeds placed in one metric, ownership measured in another | `be052528`. Guards added; the Rays one is measured **off-axis on purpose**, since the stretch leaves the four axes exactly where they are. `PointScatter.gridJitter` gained an `aspect` defaulting to `1f`, so Soft Overlaps is untouched byte for byte |
| 2 | **The Mondrian drew graph paper at the mode it opens in.** Measured: at `BICHROMATIC` — the default — the render was **96.5% bare ground**, 2.6% ink, no accent anywhere. `blockColor` opened with `palette.size <= 2 -> ground`, and the color mode reduces every palette to exactly two stops | `0971647e`. Reads the ramp through `RampTones`, dropping its final tone, which *is* the ink |
| 3 | **The radial pair had no organic axis.** Rings and Rays were rigid at every setting | `62153ac0`. Rings gained a **Wobble** and Rays an **Unevenness**. Rings scales the measured *distance* by a `SeededHarmonics` factor on the bearing — a monotonic stretch along every bearing, so ring `n` stays inside ring `n+1`, where displacing each ring's radius on its own would let two meet and tear. Rays walks each wedge edge off its even position, held in order by the travel bound alone; a first pass set that bound at `0.5`, which lets two neighbors close the whole gap and draw a zero-width ray |
| 4 | **The Truchet drew one line weight.** Its arc width was a constant | `f4ac9cb7`. A **Thickness**, curved so `0.5` resolves to the shipped `0.34` of a cell. The ends are two more designs: a fine tracery, and arcs so wide they meet and the *ground* becomes the pattern |
| 5 | **The two flat ramps could only point one way.** `LINEAR_GRADIENT` had no knobs at all; `GRADIENT_COLUMNS`, whose whole content is a direction, had no control over it | `4c16db27`. Both declare `rotation` and read it through `frameAxis`, `0` reproducing what each always drew. **A half turn, knowingly** — see the note below |
| 6 | **Rays was the loudest design in the catalog and had no way down.** Hard edges at full palette contrast filling the frame, against the first aesthetic principle | `04c8b32f`. `roundness` as **Softness**: each wedge leans into its neighbors near a seam, up to half a wedge either side. `0` is the old sunburst exactly, `1` leaves no flat interior at all — a conic gradient, a second design rather than a softened first one. **The default `0.5` is a soft-seamed sunburst on purpose**: a knob whose default reproduced the picture the finding was about would fix nothing for anyone who never drags it. Measured — the largest adjacent-pixel step away from the fan's convergence point falls from 573 of 765 to 33 at `0.5` and 17 at `1` |
| 7 | **The Voronoi's cells could be painted in its own leading.** Cells and seams read the same palette, so the bottom of the frame arrived at the seam's own color and the mosaic lost its seams exactly there — flat blobs, the one thing the seams exist to prevent | `f67470fb`. The fill reads the ramp through `fillCeiling`, stopping one of `RampTones`' own tones short of the last stop. Taking the step from there rather than inventing a margin is the point, and it is what handles the two-stop reduction a fixed fraction would get wrong |
| 8 | **The Voronoi spent its palette exactly one way** — the ramp at the seed's height, where every driven design got a `colorLayout` chooser | `b7da8c62`. Three: **Vertical** (what it always drew, so `0` moves no stored recipe), **Radial** (distance from the middle over distance to a corner — the layout that stops the mosaic reading as a gradient someone cut up), **Scattered** (a draw per cell, the stained-glass reading). The scatter runs on a salted stream drawn for *every* cell whatever the layout is, or picking a layout would rearrange the mosaic underneath a knob labelled *Colors* |
| 9 | **Plasma had one knob and no organic axis.** Four sines at one frequency interfere perfectly regularly however the phases fall, so the field repeated across the frame and read as a pattern rather than a fluid | `6ff3021b`. `irregularity` as **Turbulence**, warping the plane before `sample` sees the point. The push is measured in **wavelengths**, not frame widths — the frequency knob spans a four-fold range, so a fixed distance would be a nudge at one end and noise at the other, one slider changing what the slider beside it means. Measured: the share of spectral energy in the 64 strongest bins falls from `0.785` to `0.540` |

**The domain warp is shared now.** Metaballs had two salted noise fields inline; Plasma was the second consumer, so
the mechanism moved to `DomainWarp` — including the reason there are *two* fields, since one pushes every point along
the diagonal and the picture combs instead of swirling. `reach` stays in the caller's own units (Metaballs pushes a
share of the frame, Plasma a share of a wavelength) because the two genuinely disagree about what those are. Metaballs
is unchanged by it: all 32 of its harness renders are byte-identical, worst channel difference 0.

**The half turn on the two ramps is a knowing limit, and the better answer is a change to the guard.** A ramp is
*directed* — light-at-the-top-right and light-at-the-bottom-left are one axis and two pictures — so covering every
direction takes a full turn, and a full turn makes `rotation = 1` wrap onto `rotation = 0`.
`GeneratorKnobTest.everyDeclaredKnobChangesThePicture` fails that, correctly: a knob whose two ends render identically
is the dead knob it exists to catch. Both readings are right. What is missing is a way for a generator to declare **the
period of its angle**, so the guard compares two points that are not a whole turn apart — a change to `DesignStyle` and
the guard, and a separate idea from this pass.
...
**Two of the nine no longer exist**, so findings 1, 3 and 6 above are partly a record of work that has since been
deleted. They are kept as written rather than trimmed: the aspect-metric bug in finding 1 was the same bug in five
designs and three of them still carry the guard, and finding 6 is the clearest statement in this document of *why* a
knob's default has to be the fixed picture rather than the old one.
...
| 11 | **Both radial designs drew a target, which is not a wallpaper.** Rings put a bullseye in the middle of the icon grid and Rays a pinwheel, each with a point of convergence the eye cannot leave alone. Rays' new *Softness* could not touch it — a convergence point is not an edge — which is what made this a **concept** finding rather than a quality one, and the author's call was improve it or drop both | **Dropped**, in two steps. First the concept was fixed: the cause was one line in each, an inset keeping the origin *inside* the frame, and moving it outside turned the target into a sweep — arcs crossing the screen, a fan of light entering from one side. That worked, and it is what settled the question the other way: with the origin off the frame, Rings is a flowing contour field that `WAVES` and `RIBBON_FLOW` already cover, and neither design is worth a slot for it. `RINGS` and `RAYS` are removed, along with the `OffFrameOrigin` the rework introduced — a shared type with no consumers left is a model in a vacuum |
...
3. ~~**How many of the missing 9 designs to build**~~ — **settled.** All of them but one: the thin-line family it
